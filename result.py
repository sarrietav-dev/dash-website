...
df_res = pd.read_sql_query(
    'select * from "vw_offcorss_descar_clu_agr"', con=engine)
cluster_names_p3 = {0: "sale_hunters", 1: "average_customer",
...
# Agrupación 1
df_grupo_art = df_res2.groupby(
    ["genero", "edad", "grupo_articulo", "clu_name"]).sum().reset_index()
...
tabla_grupo_art = df_grupo_art[(df_grupo_art["genero"] == gen) &
                               ...
                               (df_grupo_art["clu_name"] == clu)][["grupo_articulo", "cantidad", "freq_relativa"]]\
    .reset_index(drop=True).sort_values(by="cantidad", ascending=False)

# Agrupación 2
df_tipo_art = df_res2.groupby(["genero", "edad", "grupo_articulo",
                               "tipo_articulo", "tipo_tejido", "clu_name"]).sum().reset_index()

tabla_tipo_art = df_tipo_art[(df_tipo_art["genero"] == gen) &
                             (df_tipo_art["edad"] == ed) &
                             (df_tipo_art["clu_name"] == clu) &
                             (df_tipo_art["grupo_articulo"] == ga)][["tipo_articulo", "cantidad", "freq_relativa", "tipo_tejido"]]\
    .reset_index(drop=True).sort_values(by="cantidad", ascending=False)
...
dropdown_prod = dcc.Dropdown(
    ...
    options=[{"label": e, "value": e}
             for e in df_res2["grupo_articulo"].sort_values().unique()],
    searchable=False,
    style={'height': '30px', 'width': '300px'}
...
dropdown_top = dcc.Dropdown(
    ...
    value=[],
    className="dropdown m-3",
    options=[{"label": "Top 10", "value": "head"},
             {"label": "Bottom 5", "value": "tail"}],
    searchable=False,
